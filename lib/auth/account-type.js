const TEACHER_ONLY_PREFIXES = ["/classes", "/dashboard", "/teachers"];

export function parseAccountType(value) {
  if (value === "student") return "student";
  if (value === "teacher") return "teacher";
  if (value === "player") return "player";
  return null;
}

export function normalizeAccountType(value) {
  return parseAccountType(value) || "student";
}

export function isTeacherAccountType(value) {
  return parseAccountType(value) === "teacher";
}

export function isStudentAccountType(value) {
  return parseAccountType(value) === "student";
}

export function isArcadeAccountType(value) {
  const parsed = parseAccountType(value);
  return parsed === "student" || parsed === "player";
}

function getStudentNumberFromUser(user) {
  const emailLocalPart = String(user?.email || "").split("@")[0].trim();
  return emailLocalPart.match(/^(?:s-)?([0-9]{7})$/i)?.[1] || "";
}

function isStudentIdentifierDisplayName(user, value) {
  const studentNumber = getStudentNumberFromUser(user);
  if (!studentNumber) return false;

  const compact = String(value || "").trim().toLowerCase().replace(/[\s_-]+/g, "");
  return compact === studentNumber || compact === `s${studentNumber}`;
}

export function buildDefaultDisplayName(user) {
  const meta = user?.user_metadata || {};
  const emailLocalPart = String(user?.email || "").split("@")[0].trim();
  const studentNumber = getStudentNumberFromUser(user);

  const metadataCandidates = [
    meta.display_name,
    meta.full_name,
    meta.name,
    [meta.given_name, meta.family_name].filter(Boolean).join(" "),
    meta.user_name,
  ];
  const metadataName = metadataCandidates
    .map((value) => String(value || "").trim())
    .find((value) => value && !isStudentIdentifierDisplayName(user, value));

  if (metadataName) return metadataName;
  if (emailLocalPart && !isStudentIdentifierDisplayName(user, emailLocalPart)) return emailLocalPart;
  return studentNumber ? "Student" : "MathClaw User";
}

export function splitDisplayName(displayName) {
  const normalized = String(displayName || "").trim().replace(/\s+/g, " ");
  if (!normalized) {
    return { firstName: "", lastName: "" };
  }

  const [firstName, ...rest] = normalized.split(" ");
  return {
    firstName,
    lastName: rest.join(" "),
  };
}

export function getPublicDisplayName(profileLike, fallback = "Student") {
  const nickname = String(profileLike?.nickname || "").trim();
  if (nickname) return nickname;

  const displayName = String(profileLike?.display_name || profileLike?.displayName || "").trim();
  if (displayName) return displayName;

  const fallbackValue = String(fallback ?? "").trim();
  if (fallbackValue) return fallbackValue;
  return arguments.length > 1 ? "" : "Student";
}

export async function getAccountTypeForUser(supabase, user, fallback = "student") {
  if (!user) return fallback;

  const metadataType = parseAccountType(user.user_metadata?.account_type);

  const { data, error } = await supabase
    .from("profiles")
    .select("account_type")
    .eq("id", user.id)
    .maybeSingle();

  if (!error) {
    const profileType = parseAccountType(data?.account_type);
    if (profileType) return profileType;
  }

  const [{ data: joinedMembership }, { data: ownedCourse }] = await Promise.all([
    supabase
      .from("student_course_memberships")
      .select("id")
      .eq("profile_id", user.id)
      .limit(1)
      .maybeSingle(),
    supabase
      .from("courses")
      .select("id")
      .eq("owner_id", user.id)
      .limit(1)
      .maybeSingle(),
  ]);

  if (joinedMembership && !ownedCourse) {
    return "student";
  }

  if (ownedCourse) {
    return "teacher";
  }

  return metadataType || fallback;
}

export async function ensureProfileForUser(supabase, user, accountType) {
  if (!user) return;

  const safeAccountType = normalizeAccountType(accountType);
  const defaultDisplayName = buildDefaultDisplayName(user);
  const metadataSchoolName = String(user.user_metadata?.school_name || "").trim() || null;

  const { data: existingProfile } = await supabase
    .from("profiles")
    .select("*")
    .eq("id", user.id)
    .maybeSingle();

  const existingDisplayName = String(existingProfile?.display_name || "").trim();
  const profileDisplayName =
    existingDisplayName && !isStudentIdentifierDisplayName(user, existingDisplayName)
      ? existingDisplayName
      : defaultDisplayName;

  const discoverable =
    safeAccountType === "teacher"
      ? typeof existingProfile?.discoverable === "boolean"
        ? existingProfile.discoverable
        : true
      : false;

  const basePayload = {
    id: user.id,
    display_name: profileDisplayName,
    nickname: existingProfile?.nickname || null,
    school_name: existingProfile?.school_name ?? metadataSchoolName,
    timezone: existingProfile?.timezone || "America/New_York",
    discoverable,
    account_type: safeAccountType,
    updated_at: new Date().toISOString(),
  };

  let { error } = await supabase.from("profiles").upsert(basePayload, { onConflict: "id" });

  if (
    error &&
    (error.code === "42703" ||
      (typeof error.message === "string" && error.message.includes("nickname")))
  ) {
    const { nickname, ...payloadWithoutNickname } = basePayload;
    const retry = await supabase
      .from("profiles")
      .upsert(payloadWithoutNickname, { onConflict: "id" });
    error = retry.error;
  }

  // Postgres error code 42703 = undefined_column (old schema missing account_type).
  // Only fall back to omitting account_type for that specific error, not for check constraint
  // violations (23514), which should no longer occur now that 'player' is in the constraint.
  const isUndefinedColumnError =
    error &&
    (error.code === "42703" ||
      (typeof error.message === "string" &&
        error.message.includes("account_type") &&
        !error.message.includes("violates check constraint")));

  if (isUndefinedColumnError) {
    const legacyPayload = {
      id: user.id,
      display_name: profileDisplayName,
      nickname: existingProfile?.nickname || null,
      school_name: existingProfile?.school_name ?? metadataSchoolName,
      timezone: existingProfile?.timezone || "America/New_York",
      discoverable,
      updated_at: new Date().toISOString(),
    };
    let retry = await supabase.from("profiles").upsert(legacyPayload, { onConflict: "id" });
    if (
      retry.error &&
      typeof retry.error.message === "string" &&
      retry.error.message.includes("nickname")
    ) {
      const { nickname, ...legacyWithoutNickname } = legacyPayload;
      retry = await supabase
        .from("profiles")
        .upsert(legacyWithoutNickname, { onConflict: "id" });
    }
    error = retry.error;
  }

  if (error && typeof error.message === "string" && error.message.includes("discoverable")) {
    const olderPayload = {
      id: user.id,
      display_name: profileDisplayName,
      school_name: existingProfile?.school_name ?? metadataSchoolName,
      timezone: existingProfile?.timezone || "America/New_York",
      updated_at: new Date().toISOString(),
    };
    await supabase.from("profiles").upsert(olderPayload, { onConflict: "id" });
  }
}

export function defaultNextForAccountType(accountType) {
  return isTeacherAccountType(accountType) ? "/classes" : "/play";
}

export function sanitizeNextForAccountType(next, accountType) {
  const safeAccountType = normalizeAccountType(accountType);
  const fallback = defaultNextForAccountType(safeAccountType);

  if (!next || typeof next !== "string" || !next.startsWith("/")) {
    return fallback;
  }

  if (
    !isTeacherAccountType(safeAccountType) &&
    TEACHER_ONLY_PREFIXES.some(
      (prefix) => next === prefix || next.startsWith(`${prefix}/`)
    )
  ) {
    return "/play";
  }

  return next;
}
