...
import { DEFAULT_ASSIGNMENT, INTEGER_LEVELS, INTEGER_LEVEL_MAP, getLevelById } from "./levels";
...
export function createProblem(level, options = {}) {
  ...
  return {
    ...
    answerNode: buildIntegerNode(problem.answer),
    counters: buildCounters(problem),
    numberLine: buildNumberLine(problem),
    strategy,
    choices:
      level.scaffolds.answerMode === "multiple_choice"
        ? buildDistractors(problem, level.scaffolds.choiceCount || 4, level.distractorProfile)
        : [],
  };
}
...
export function evaluateLevelMastery(level, profileSummary) {
  const focusAccuracies = level.skillTags
    .map((tag) => profileSummary.accuracyBySkill?.[tag]?.accuracy)
    .filter((value) => typeof value === "number");
  const focusAccuracy = focusAccuracies.length ? average(focusAccuracies) : profileSummary.last10Accuracy;
  const pass =
...
export function chooseRemediationSkill(profileSummary, level) {
...
export function deriveTemporaryScaffolds(level, profileSummary) {
  const weakSkill = chooseRemediationSkill(profileSummary, level);
...
export function applyAnswerToProfile(profile, answerEntry) {
  ...
  nextProfile.rollingHistory = pushLimited(nextProfile.rollingHistory, answerEntry, 120);
...
export function computeLevelChange({
  ...
  const mastery = evaluateLevelMastery(level, profileSummary);
  ...
  if (mode === "assignment") {
    ...
    return {
      nextLevelId,
      mastery,
      supportScaffolds: assignment.allowAdaptiveSupport ? deriveTemporaryScaffolds(level, profileSummary) : null,
    };
  }
  ...
  if (mode === "progression") {
    return {
      ...
      mastery,
      supportScaffolds: deriveTemporaryScaffolds(level, profileSummary),
    };
  }

  const supportScaffolds = deriveTemporaryScaffolds(level, profileSummary);
  if (mastery.pass) {
