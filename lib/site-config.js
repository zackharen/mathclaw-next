...
const SITE_CONFIG_READ_TIMEOUT_MS = 1500;

function timeoutAfter(ms) {
  return new Promise((_, reject) => {
    setTimeout(() => reject(new Error("Site config read timed out")), ms);
  });
}
...
const listCachedSiteConfigRows = unstable_cache(
  async () => listSiteConfigRows(createAdminClient()),
  ["site-config-rows-v1"],
  { revalidate: 60, tags: [SITE_CONFIG_CACHE_TAG] }
);

function getSiteConfigRows(admin) {
  return admin ? listSiteConfigRows(admin) : listCachedSiteConfigRows();
}
...
export async function getSiteFeatureConfig(admin = null) {
  try {
    const rows = await Promise.race([
      getSiteConfigRows(admin),
      timeoutAfter(SITE_CONFIG_READ_TIMEOUT_MS),
    ]);
...
export async function getSiteCopy(admin = null) {
  try {
    const rows = await Promise.race([
      getSiteConfigRows(admin),
      timeoutAfter(SITE_CONFIG_READ_TIMEOUT_MS),
