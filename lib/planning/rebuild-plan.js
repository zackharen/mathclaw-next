...
export async function rebuildPlanFromCalendar({
  ...
  const [access, calendarRes] = await Promise.all([
    ...
    supabase
      ...
      .select("class_date, ab_day, day_type, is_grace_day, lesson_count_override")
  ...
  const forceFullRebuild =
    ...
    filteredDays.some((day) => Number.isInteger(day.lesson_count_override));
  ...
  if (!effectiveStartDate) {
    ...
    if (pacingMode === "manual_complete") {
      const cappedCompleted = Math.min(completedCount, lessonCount);
      let completedPointer = 0;
      for (const day of filteredDays) {
        ...
        for (let slot = 1; slot <= dailyLessonCount; slot += 1) {
          const isCompleted = completedPointer < cappedCompleted;
          const lessonIndex = isCompleted
            ? completedPointer
            : cappedCompleted + slot - 1;
          if (lessonIndex >= lessonCount) break;
          rowsToInsert.push({
            course_id: course.id,
          ...
          if (isCompleted) completedPointer += 1;
        }
      }
    } else {
      let lessonPointer = 0;
