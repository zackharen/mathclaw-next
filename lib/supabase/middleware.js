...
function timeoutAfter(ms) {
  return new Promise((resolve) => {
    setTimeout(() => resolve({ data: { user: null }, error: new Error("Auth lookup timed out") }), ms);
  });
}
...
export async function updateSession(request) {
  ...
  try {
    ...
    const {
      data: { user },
    } = await Promise.race([supabase.auth.getUser(), timeoutAfter(AUTH_LOOKUP_TIMEOUT_MS)]);
