...
function appendUniqueChoice(choices, value) {
  if (Number.isFinite(value) && !choices.includes(value)) {
    choices.push(value);
  }
}

function inferPercentDirection(correctAnswer, scale) {
  return Number(correctAnswer) < scale ? "decrease" : "increase";
}

function buildRandomMultiplierChoice({ scale, usedChoices }) {
  const maxChoice = scale * 2 - 1;

  for (let attempt = 0; attempt < 25; attempt += 1) {
    const choice = randomBetween(1, maxChoice);
...
function buildPercentMultiplierChoices({ correctAnswer, percentValue, direction, scale }) {
  const rawPercent = Number(percentValue || 0) / MULTIPLIER_SCALE;
  const percentOffset = Math.round(rawPercent * scale);
  const safeDirection = direction === "increase" || direction === "decrease"
    ? direction
    : inferPercentDirection(correctAnswer, scale);
  const oppositeAnswer = safeDirection === "decrease"
    ? scale + percentOffset
    : scale - percentOffset;
  const decimalPercentAnswer = percentOffset;
  const orderedChoices = [];

  appendUniqueChoice(orderedChoices, correctAnswer);
  appendUniqueChoice(orderedChoices, oppositeAnswer);
  appendUniqueChoice(orderedChoices, decimalPercentAnswer);
  appendUniqueChoice(
    orderedChoices,
    buildRandomMultiplierChoice({ scale, usedChoices: orderedChoices })
  );

  for (let choice = 1; orderedChoices.length < 4 && choice <= scale * 2 - 1; choice += 1) {
    appendUniqueChoice(orderedChoices, choice);
  }
...
export function buildDoubleBoardMultipleChoiceOptions(question = {}) {
  ...
  if (metadata.answerFormat === "multiplier_hundredths") {
    return buildPercentMultiplierChoices({
      correctAnswer,
      percentValue: metadata.percentValue || question.operand1,
  ...
  if (metadata.answerFormat === "multiplier_tenthousandths") {
    return buildPercentMultiplierChoices({
      ...
      percentValue: metadata.percentValue || question.operand1,
      direction: metadata.direction,
      scale: DECIMAL_MULTIPLIER_SCALE,
    });
  }
