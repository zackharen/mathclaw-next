...
function deriveVisuals(state, now) {
  ...
  if (state.enemyState === "windup") {
    ...
    enemyCenterX += (state.telegraphSide === "left" ? -12 : 12) + (state.telegraphSide === "left" ? -4 : 4) * hopProgress;
    ...
    enemyCenterX += state.pendingAttack?.side === "left" ? -18 : 18;
    enemyCenterY -= 12;
    enemyScale = 1.06;
...
export function buildShowdownTutorialScenario(step = "dodge", difficultySlug = "easy", now = Date.now()) {
  ...
  if (step === "jab") {
    const tutorialAttack = {
      ...base.pendingAttack,
      recoveryMs: Math.max(base.pendingAttack?.recoveryMs || 900, 1500),
    };
    ...
    return withVisuals(
      {
        ...
        enemyState: "recovery",
        enemyStateStartedAt: now,
        enemyStateEndsAt: now + tutorialAttack.recoveryMs,
        pendingAttack: tutorialAttack,
        attackLabel: "Counter Drill",
        effectText: "Good dodge. Larry is open now. Press Space or Z to jab.",
  ...
  return withVisuals(
    {
      ...
      effectText: "Watch Larry hop left, then dodge right when he comes in.",
    },
    now
  );
}
