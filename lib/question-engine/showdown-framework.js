...
const PLAYER_ACTION_MS = 260;
const PLAYER_STUN_MS = 260;
const BLOCK_WINDOW_MS = 260;
const POST_HIT_DELAY_MS = 220;
...
function buildTelegraphState(patternIndex, now, previousAttackLabel = "") {
...
function deriveVisuals(state, now) {
  ...
  if (state.enemyState === "windup") {
    ...
    enemyCenterX += state.telegraphSide === "left" ? -18 : 18;
    enemyRotation = state.telegraphSide === "left" ? -7 : 7;
...
export function initialShowdownState(now = Date.now()) {
  const telegraph = buildTelegraphState(0, now);
...
export function performPlayerAction(state, action, now = Date.now()) {
  ...
  if (action === "dodge_left" || action === "dodge_right") {
    nextState = {
      ...
      playerState: action,
      playerActionEndsAt: now + PLAYER_ACTION_MS,
  ...
  if (action === "block") {
    nextState = {
      ...
      attempts: nextState.attempts + 1,
      playerState: "block",
      playerActionEndsAt: now + BLOCK_WINDOW_MS,
      effectText: "Guard up.",
    };
    return withVisuals(nextState, now);
  }
  ...
  return withVisuals(
    {
      ...
      playerState: "jab",
      playerActionEndsAt: now + PLAYER_ACTION_MS,
      playerRecoverUntil: now + PLAYER_STUN_MS,
...
export function stepShowdownFight(state, now = Date.now()) {
  ...
  if (nextState.enemyState === "recovery" || nextState.enemyState === "stunned") {
    ...
    const telegraph = buildTelegraphState(nextIndex, now, nextState.attackLabel);
    nextState = {
      ...nextState,
