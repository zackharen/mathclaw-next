...
export function serializeSpiralReviewQuestion(question) {
  return {
    skill: question.skill,
    prompt: question.prompt,
    promptNode: question.promptNode || null,
    answer: question.answer,
    choices: question.choices,
    choiceNodes: question.choices.map((choice) =>
      question.formatChoiceNode ? question.formatChoiceNode(choice) : buildLabelNode(choice)
    ),
    explanation: question.explanation,
    leftLabel: question.leftLabel || null,
    rightLabel: question.rightLabel || null,
    leftNode: question.leftNode || null,
    rightNode: question.rightNode || null,
  };
}

export function hydrateSpiralReviewQuestion(question) {
  return {
    ...question,
    formatChoiceNode(choice) {
      const choiceIndex = question.choices.findIndex((candidate) => candidate === choice);
      return question.choiceNodes[choiceIndex] || buildLabelNode(choice);
    },
    checkAnswer(value) {
      return question.skill === "integers"
        ? Number(value) === Number(question.answer)
