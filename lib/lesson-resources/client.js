...
export async function postLessonResource(body) {
  ...
  if (!response.ok) {
    const error = new Error(data.error || "Lesson resources could not be updated.");
...
export async function uploadLessonVocabularyFile({
  ownerId,
  courseId,
  classDate,
  lessonIds,
  ...
  try {
    return await postLessonResource({
      action: "register-vocabulary-file",
      courseId,
      classDate,
      lessonIds,
      word,
...
export async function uploadLessonVocabularyImage({
  ownerId,
  courseId,
  resourceId,
  lessonIds,
  ...
  try {
    return await postLessonResource({
      action: "register-vocabulary-image",
      courseId,
      resourceId,
      lessonIds,
      word,
