...
const RESOURCE_SELECT =
  "id, owner_id, item_kind, resource_type, title, definition, url, storage_bucket, storage_path, file_name, mime_type, size_bytes, created_at";
const LEGACY_RESOURCE_SELECT =
...
export async function loadLessonResourcePlanningData({ userId, lessonIds }) {
  ...
  let resourcesResult = await admin
    .from("lesson_resources")
    .select(RESOURCE_SELECT)
    .in("id", resourceIds);
  let vocabularyAvailable = true;
  if (resourcesResult.error && String(resourcesResult.error.message || "").includes("item_kind")) {
    vocabularyAvailable = false;
    resourcesResult = await admin
      .from("lesson_resources")
      .select(LEGACY_RESOURCE_SELECT)
      .in("id", resourceIds);
  }
  const { data: directShares, error: sharesError } = await admin
    .from("lesson_resource_shares")
    .select("resource_id, teacher_id")
  ...
  const resourcesError = resourcesResult.error;
