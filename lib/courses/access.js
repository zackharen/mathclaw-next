...
export async function listEditableCoursesForUser(
  ...
  if (!includeArchived && columns === rpcColumns) {
  ...
  if (sharedCourseIds.length > 0) {
    let sharedCoursesQuery = supabase
      .from("courses")
      .select(columns)
      .in("id", sharedCourseIds);
    if (!includeArchived) sharedCoursesQuery = sharedCoursesQuery.is("archived_at", null);
    const { data: sharedCourses, error: sharedCoursesError } = await sharedCoursesQuery;
  ...
  if (!adminContext.isOwner && adminContext.isAdmin && adminContext.schoolName) {
    ...
    if (teacherIds.length > 0) {
      let schoolCoursesQuery = admin
        .from("courses")
        .select(columns)
        .in("owner_id", teacherIds);
      if (!includeArchived) schoolCoursesQuery = schoolCoursesQuery.is("archived_at", null);
      const { data: schoolCourses, error: schoolCoursesError } = await schoolCoursesQuery;
