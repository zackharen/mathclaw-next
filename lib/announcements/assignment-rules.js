...
function pickEvenlyInside(items, count) {
  ...
  for (let i = 1; i <= count; i += 1) {
    const index = Math.max(
      ...
      Math.min(
        items.length - 1,
        Math.round((i * (items.length + 1)) / (count + 1)) - 1
      )
    );
    if (!seen.has(index)) {
  ...
  for (const rule of rules || []) {
    ...
    if (rule.cadence === "marking_period") {
      ...
      for (const period of markingPeriodRules || []) {
        ...
        // interior picker avoids anchoring the first occurrence to the period/rule start.
        ...
        for (const date of pickEvenlyInside(resolvedDates, count)) {
          addOccurrence(date, rule);
        }
      }
    }
