  ...
  return (assignments || [])
    .map((assignment) => {
      ...
      if (assignmentDate < classDate && dueDate) {
        return `${label} | Assigned ${formatShortDate(assignmentDate)} | Due ${formatShortDate(dueDate)}`;
      }
      if (assignmentDate > classDate) {
        return dueDate
          ? `${label} | Assigned ${formatShortDate(assignmentDate)} | Due ${formatShortDate(dueDate)}`
          : `${label} | ${formatShortDate(assignmentDate)}`;
      }
      return dueDate ? `${label} | Due ${formatShortDate(dueDate)}` : label;
    })
    .filter(Boolean)
    .join("\n");
}
