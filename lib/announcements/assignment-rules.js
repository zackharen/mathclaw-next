...
function pickEvenlyAfterStart(items, count) {
  if (items.length <= count) return items;
  if (count <= 1) return [items[Math.floor(items.length / 2)]];
  const picked = [];
  const seen = new Set();
  const firstIndex = Math.max(1, Math.floor((items.length - 1) / (count + 1)));
  const availableSpan = items.length - 1 - firstIndex;
  for (let i = 0; i < count; i += 1) {
    const index = Math.round(firstIndex + (i * availableSpan) / (count - 1));
    if (!seen.has(index)) {
    ...
    occurrences.push({
      ...
      due_date:
        Number.isInteger(dueSchoolDays) && dueSchoolDays > 0
          ...
          : null,
      one_lesson_on_assignment_day:
        rule.settings?.one_lesson_on_assignment_day === true &&
        !(Number.isInteger(dueSchoolDays) && dueSchoolDays > 0),
      label: rule.label,
  ...
  for (const rule of rules || []) {
    ...
    if (rule.cadence === "marking_period") {
      ...
      for (const period of markingPeriodRules || []) {
        const dueSchoolDays = Number.parseInt(String(settings.due_school_days || ""), 10);
        const dueDayBuffer = Number.isInteger(dueSchoolDays) && dueSchoolDays > 0
          ? dueSchoolDays
          : 0;
        const lastAllowedDayNumber = Math.max(
          period.start_day_number,
          period.end_day_number - 5 - dueDayBuffer
        );
        // Candidate dates: all school days in the period matching weekday filter, then shift to meeting day.
        // The final five school days (plus any due-date offset) are reserved, and the
        // spacing avoids anchoring the first occurrence to the period/rule start while
        // still using the latest safe date before the final-week deadline.
        const candidateDates = allSchoolDates.filter((date) => {
          if (ruleStartDate && date < ruleStartDate) return false;
          const dayNumber = Number(schoolDayNumberByDate.get(date));
          if (!dayNumber || dayNumber < period.start_day_number || dayNumber > lastAllowedDayNumber) {
            return false;
          }
        ...
        const resolvedDates = [...new Set(
          candidateDates
            ...
            .filter((date) => {
              if (!date || (ruleStartDate && date < ruleStartDate)) return false;
              const dayNumber = Number(schoolDayNumberByDate.get(date));
              return dayNumber >= period.start_day_number && dayNumber <= lastAllowedDayNumber;
            })
        )];
        for (const date of pickEvenlyAfterStart(resolvedDates, count)) {
          addOccurrence(date, rule);
        }
      }
    }
...
export function numberRuleAssignmentOccurrences(occurrences, markingPeriodRules = []) {
  const orderedPeriods = [...(markingPeriodRules || [])].sort(
    (a, b) => Number(a.start_day_number || 0) - Number(b.start_day_number || 0)
  );
  const periodNumberByName = new Map(
    orderedPeriods.map((period, index) => [period.name || period.id, index + 1])
  );
  ...
  return [...(occurrences || [])]
    ...
    .map((occurrence) => {
      const markingPeriodNumber = periodNumberByName.get(occurrence.marking_period) || 1;
      const counterKey = `${occurrence.rule_id || occurrence.label}|${markingPeriodNumber}`;
      const sequence = (counters.get(counterKey) || 0) + 1;
      counters.set(counterKey, sequence);
      return {
        ...occurrence,
        marking_period_number: markingPeriodNumber,
        assessment_sequence: sequence,
        assessment_number: `${markingPeriodNumber}.${sequence}`,
      };
    })
    .sort((a, b) => {
      if (a.assignment_date !== b.assignment_date) return a.assignment_date.localeCompare(b.assignment_date);
...
export function buildRuleAssignmentsByDate(options) {
  ...
  const numbered = numberRuleAssignmentOccurrences(
    buildRuleAssignmentOccurrences({ ...options, includeSkipped: true }),
    options.markingPeriodRules
  );
  for (const occurrence of numbered) {
    if (occurrence.is_skipped) continue;
    const arr = assignmentsByDate.get(occurrence.assignment_date) || [];
    if (!arr.some((assignment) => assignment.rule_id === occurrence.rule_id && assignment.original_date === occurrence.original_date)) {
      arr.push({
        rule_id: occurrence.rule_id,
        assignment_date: occurrence.assignment_date,
        original_date: occurrence.original_date,
        label: `${occurrence.label} ${occurrence.assessment_number}`,
        base_label: occurrence.label,
...
export function selectUpcomingRuleAssignments({
  assignmentsByDate,
  classDate,
  calendarDays,
  lookaheadSchoolDays = 5,
}) {
  const schoolDates = (calendarDays || [])
    .filter((day) => day.day_type !== "off")
    .map((day) => day.class_date)
    .filter(Boolean)
    .sort();
  const currentIndex = schoolDates.indexOf(classDate);
  const cutoffDate = currentIndex === -1
    ? classDate
    : schoolDates[Math.min(currentIndex + lookaheadSchoolDays, schoolDates.length - 1)];
  const upcoming = [];

  for (const assignments of assignmentsByDate?.values?.() || []) {
    for (const assignment of assignments || []) {
      const assignmentDate = assignment.assignment_date;
      const isActive = assignmentDate < classDate && assignment.due_date >= classDate;
      const isScheduledSoon = assignmentDate >= classDate && assignmentDate <= cutoffDate;
      if (isActive || isScheduledSoon) upcoming.push(assignment);
    }
  }
...
export function formatAnnouncementAssignmentDate(isoDate) {
  const [year, month, day] = String(isoDate || "").split("-").map(Number);
...
export function formatUpcomingRuleAssignments(assignments, classDate) {
  return (assignments || [])
    .map((assignment) => {
      ...
      if (assignmentDate < classDate && dueDate) {
        return `${label} | Assigned ${formatAnnouncementAssignmentDate(assignmentDate)} | Due ${formatAnnouncementAssignmentDate(dueDate)}`;
      }
      if (assignmentDate > classDate) {
        return dueDate
          ? `${label} | Assigned ${formatAnnouncementAssignmentDate(assignmentDate)} | Due ${formatAnnouncementAssignmentDate(dueDate)}`
          : `${label} | ${formatAnnouncementAssignmentDate(assignmentDate)}`;
      }
      return dueDate ? `${label} | Due ${formatAnnouncementAssignmentDate(dueDate)}` : label;
    })
    .filter(Boolean)
    .join("\n");
}

export function buildOneLessonAssignmentDates(options) {
  return new Set(
    buildRuleAssignmentOccurrences(options)
      .filter((occurrence) => occurrence.one_lesson_on_assignment_day)
      .map((occurrence) => occurrence.assignment_date)
  );
}
