...
export async function listGamesWithCourseSettings(supabase, courseId = null, options = {}) {
  ...
  const [gamesResult, featureConfig] = await Promise.all([
    supabase
      .from("games")
      .select("slug, name, category, description, is_multiplayer")
      .order("name"),
    getSiteFeatureConfig().catch(() => ({ audienceBySlug: {} })),
  ]);
  const { data: dbGames, error: gamesError } = gamesResult;
  ...
  const mergedGames = sortGamesByName(
    GAME_CATALOG.map((game) => {
      ...
      return dbMatch ? { ...dbMatch, ...game } : game;
    })
  );
...
export async function filterCoursesForGame(courses, gameSlug, options = {}) {
  ...
  try {
    featureConfig = await getSiteFeatureConfig();
  } catch (error) {
