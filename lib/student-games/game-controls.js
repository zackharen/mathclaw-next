...
export async function listGamesWithCourseSettings(supabase, courseId = null) {
  ...
  if (!courseId || !Array.isArray(mergedGames) || mergedGames.length === 0) {
    return withEnabledState(mergedGames, new Map(), null);
  }
  ...
  try {
    ...
    if (settingsError) {
      if (isMissingSettingsTableError(settingsError)) {
        return withEnabledState(mergedGames, new Map(), courseId);
      }
    ...
    return withEnabledState(mergedGames, settingsByKey, courseId);
  } catch (error) {
