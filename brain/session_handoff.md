...
## Last Updated
2026-06-06 America/New_York (Class Plan control layout polish)
...
- Latest local work cleans up the Class Plan **Modify Calendar** control layout (`app/classes/[id]/plan/page.js`, `app/classes/[id]/plan/apply-calendar-submit.js`, `app/globals.css`): date range controls now occupy the first row; pacing mode, Modified Day Rules, and AB Schedule occupy the second row; Apply Pacing Mode, Auto-Regenerate, Apply to all my classes, and Apply Calendar Changes occupy the third row; Full Calendar Editor, Generate / Update Announcements, and Copy Calendar to Other Classes occupy the fourth row; completed-action confirmations now all render in one bottom status space instead of appearing beside individual controls.
