...
- **Projector Party built and pushed for MathClaw** (`app/projector/*`, `app/api/projector/route.js`, `app/layout.js`, `app/globals.css`, `supabase/migrations_20260601_projector_sessions.sql`, implementation commit `bd2273c`, handoff commit `f4260bd`, pushed to `origin/main`):
  ...
  - Follow-up correction changed the Projector video Storage bucket limit from a numeric byte value to Supabase's documented string form (`75MB`) and updates existing `projector-videos` bucket settings before each prepare/convert action; this fixes tiny `.mov` recordings being rejected as too large.
