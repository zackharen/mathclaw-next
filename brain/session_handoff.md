...
## Last Updated
2026-06-06 America/New_York (Dashboard class rename settings)
...
- Latest local work adds class renaming on Dashboard (`app/dashboard/page.js`, `app/classes/actions.js`, `app/globals.css`): each class card now has a collapsible **Class Settings** panel with editable **Dashboard name** (`courses.title`) and **Class label** (`courses.class_name`) fields. Saving updates the course, revalidates Dashboard/Classes/Plan/Student Progress, and returns to Dashboard with success/error notices. Verification passed: `node --check app/classes/actions.js`; `node --check app/dashboard/page.js`; `git diff --check`; `npm run build`.
