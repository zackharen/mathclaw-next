...
## Last Updated
2026-06-02 America/New_York (Projector sidebar/media/library polish)
...
- Projector Party is in code at `/projector` for teachers and `/projector/screen` for public display screens. It uses Supabase Realtime Broadcast over `projector-session-<sessionId>` and stores non-sensitive screen states in `projector_sessions`. Projector supports text/LaTeX/image/video sending, screen-recording upload conversion to projector-friendly MP4, fullscreen receiver controls, and full-viewport image/video display on live screens. The teacher dashboard has collapsible **Screen Selection**, **Room Setups**, and **Saved Items** panels; Screen Selection is open by default and includes screen targeting plus content type selection. Projector Library v1 saves reusable text/LaTeX/image/video items; saved video/GIF thumbnails are static in the tiny saved lists but still play in live screen previews. Projector Scene Library saves/restores full four-screen room setups, including intentionally cleared screens. Room Setups support teacher-created folders for classes/situations, with old setups falling under Uncategorized. Production Supabase migrations `projector_library_items`, `projector_scene_library_items`, and `projector_scene_folders` were applied successfully.
...
3. **Projector next build: Saved Item Library polish** - add edit/rename, search/filter, categories/activity types, and clearer item-vs-scene language. Load `brain/future_ideas.md` -> "Projector Classroom Display System" before implementing.
